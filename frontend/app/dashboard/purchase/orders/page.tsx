...
import { STATUS_COLORS, PO_STATUSES, formatCurrency, formatDate } from '@/lib/purchase-config';

interface PO {
  id: string; number: string; date: string; status: string; total: number;
  vendor: { id: string; name: string; company?: string };
  _count: { lines: number; goodsReceipts: number; vendorBills: number };
}
...
export default function PurchaseOrdersPage() {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    apiClient.getPurchaseOrders({ status: filter || undefined, vendorId: params.get('vendorId') || undefined })
      .then((res) => { if (res.success) setOrders(res.data); })
      .finally(() => setLoading(false));
  }, [isAuthenticated, router, filter, params]);
    ...
    <div className="min-h-screen bg-gray-50">
      <header className="bg-white shadow-sm border-b px-6 py-4">
        ...
        <div className="flex items-center justify-between mt-2">
          ...
          <Link href="/dashboard/purchase/orders/new" className="px-4 py-2 bg-primary-600 text-white rounded-md hover:bg-primary-700 text-sm font-medium">+ New PO</Link>
        </div>
      </header>
          ...
          <div className="bg-white rounded-lg border border-gray-200 overflow-hidden">
            <table className="min-w-full divide-y divide-gray-200">
              ...
              <tbody className="divide-y divide-gray-200">
                {orders.map((o) => (
                  <tr key={o.id} className="hover:bg-gray-50 cursor-pointer" onClick={() => router.push(`/dashboard/purchase/orders/${o.id}`)}>
                    <td className="px-4 py-3 text-sm font-medium text-primary-600">{o.number}</td>
                    <td className="px-4 py-3 text-sm text-gray-700">{o.vendor.name}{o.vendor.company ? ` — ${o.vendor.company}` : ''}</td>
